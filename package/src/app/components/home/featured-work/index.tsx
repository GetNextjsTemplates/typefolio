                ...
                <div className="border-x border-primary/10">
                    <div className="flex flex-col max-w-3xl mx-auto py-10 px-4 sm:px-7">
                        <div className="flex flex-col xs:flex-row gap-5 items-center justify-between">
                            <p className="text-sm text-primary uppercase font-medium">Featured work</p>
                            <Link href={"/"} className="w-fit rounded-full py-3 px-5 border border-primary/10">
                                Download Portfolio
                            </Link>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 border-t border-primary/10">
                        {featureWork?.map((value: any, index: number) => {
                            ...
                            return (
                                <div
                                    key={index}
                                    className={`flex flex-col gap-3.5 sm:gap-5 p-3.5 sm:p-6 ${isRightCol ? 'md:border-l md:border-primary/10' : ''}`}
                                >
                                    <div>
                                        <Image
                                            src={value?.image}
